...
from model import db, LightforceTherapy
import requests
...
with app.app_context():
    ...
    # First URL
    url = "https://www.chattanoogarehab.com/us/high-power-laser-therapy"
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")
    migrate = Migrate(app, db) 
    LightforceTherapy.query.delete() 
    ...
    for product_elem in soup.find_all(class_="product details product-item-details"):
        ...
        if a_tag is not None:
            ...
            lightforceTherapyProduct = LightforceTherapy(name=product_name, price=product_price, href=product_href, image=image_url, description=description_text)
            db.session.add(lightforceTherapyProduct)
            db.session.commit()
            print(lightforceTherapyProduct)
