...
with app.app_context():
    ...
    # First URL
    url = "https://www.chattanoogarehab.com/us/training-mobility"
    ...
    for product_elem in soup.find_all(class_="product details product-item-details"):
        ...
        if a_tag is not None:
            ...
            mobilityProduct = Mobility(name=product_name, price=product_price, href=product_href, image=image_url, description=description_text)
            db.session.add(mobilityProduct)
            db.session.commit()
            print(mobilityProduct)
