...
from model import db, Recoverytherm
import requests
...
with app.app_context():
    Recoverytherm.query.delete()
    BASE_URL_PATH = "https://www.therabody.com/"
    url = "https://www.therabody.com/us/en-us/products/?prefn1=productTypeMasterPLP&prefv1=recoverytherm"
        ...
        for a_tag in product_elem.find_all("a", class_="name-link", href=True):
            ...
            try:
                ...
                product_list.append({
                    ...
                    "image":image_url,
                    "category":"Recoverytherm",  
                    "brand":"Therabody" 
                    
                    
                })

                recoverythermProduct = Recoverytherm(name=product_name, price=product_price, href=product_href, description=product_description.text, image=image_url,category="Recoverytherm",  
                    brand="Therabody" 
                    )
                db.session.add(recoverythermProduct)
                db.session.commit()
                print(recoverythermProduct)
