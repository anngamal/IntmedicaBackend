...
with app.app_context():
    ...
    url = "https://www.therabody.com/us/en-us/products/?prefn1=productTypeMasterPLP&prefv1=theraface"
    ...
    for product_elem in soup.find_all(class_="product-info"):
        ...
        for a_tag in product_elem.find_all("a", class_="name-link", href=True):
            ...
            try:
                ...
                therafaceProduct = Theraface(name=product_name, price=product_price, href=product_href, description=product_description.text, image=image_url,category="Theraface",  
                    brand="Therabody" )
                db.session.add(therafaceProduct)
                db.session.commit()
                print(therafaceProduct)
