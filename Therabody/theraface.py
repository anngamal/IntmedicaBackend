...
from model import db, Theraface
import requests
    ...
    for product_elem in soup.find_all(class_="product-name-wrapper"):
        ...
        for a_tag in product_elem.find_all("a", class_="name-link", href=True):
            ...
            try:
                ...
                therafaceProduct = Theraface(name=product_name, price=product_price, classification=product_classification, href=product_href, description=product_description.text, image=image_url)
                db.session.add(therafaceProduct)
                db.session.commit()
