...
with app.app_context():
    Theracup.query.delete()
    BASE_URL_PATH = "https://www.therabody.com/"
    url = "https://www.therabody.com/us/en-us/therabody-recovery-devices/?prefn1=productTypeMasterPLP&prefv1=theracup"
    ...
    for product_elem in soup.find_all(class_="product-name-wrapper"):
        ...
        for a_tag in product_elem.find_all("a", class_="name-link", href=True):
            ...
            try:
                ...
                theracupProduct = Theracup(name=product_name, price=product_price, classification=product_classification, href=product_href, description=product_description.text, image=image_url)
                db.session.add(theracupProduct)
                db.session.commit()
