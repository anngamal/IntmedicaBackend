...
from model import db, Recoveryair
import requests
...
with app.app_context():
    Recoveryair.query.delete()
    BASE_URL_PATH = "https://www.therabody.com/"
    url = "https://www.therabody.com/us/en-us/therabody-recovery-devices/?prefn1=productTypeMasterPLP&prefv1=recoverair"
    ...
    for product_elem in soup.find_all(class_="product-name-wrapper"):
        ...
        for a_tag in product_elem.find_all("a", class_="name-link", href=True):
            ...
            try:
                ...
                recoveryairProduct = Recoveryair(name=product_name, price=product_price, classification=product_classification, href=product_href, description=product_description.text, image=image_url)
                db.session.add(recoveryairProduct)
                db.session.commit()
